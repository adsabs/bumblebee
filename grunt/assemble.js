...
module.exports = function(grunt) {
  grunt.registerTask(
    'assemble',
    'Prepares release for distribution',
    function() {
      ...
      if (m.length !== 1) {
        throw new Error(
          'I cant find paths section, and refuse to continue for fear of breaking somehting'
        );
      }
      ...
      grunt.file.write(
        'dist/libs/g.' + version + '.js',
        grunt.file.read('dist/libs/g.js')
      );
      ...
      console.log('Updated: ' + tgt + ' section paths, with bust cache hashes');

      // now update the html
      var indexHtml = grunt.file.read('dist/index.original.html');

      // first the js path
      var newHtml = indexHtml.replace(
        /(bumblebee_app|discovery\.config)/g,
        'bumblebee_app.' + version
      );
      // then also the css
