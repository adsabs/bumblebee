...
//for the User Collection
    ...
    //XXX:alex - when saving USER_DATA it will containt target: USER_DATA; that seems unnecessary...
    Config = [
      {target : "TOKEN"},
      {target : "USER"},
      {target : "USER_DATA"}
    ...
    User = GenericModule.extend({
      ...
      setMyADSData : function(data) {
        return this.postData("USER_DATA", _.extend(this.collection.get("USER_DATA").toJSON(), data));
      },
