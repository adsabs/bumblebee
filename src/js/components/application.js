...
define([
  ...
  _.extend(Application.prototype, {
    ...
    loadModules: function(config, options) {
      ...
      var core = config['core'];config['core'];
      if (core) {
        _.each(_.keys(core), function(name) {
      ...
      var bigPromise = $.when.apply($, promises)
        ...
        .fail(function () {
          console.error("Generic error - we were not successul in loading all modules for config", config, arguments);
        });
    ...
    /*
    *
    *  load a single module (in current usage, a widget).
    *  Core objects have already been
    *  loaded using _loadModules
    *
    * */
    ...
    _loadModule : function (moduleName) {
      ...
      var d = $.Deferred();
      var path = this.config.widgets[moduleName];

      var success = function success (module) {

       this.__widgets.remove(moduleName, module);
       this.__widgets.add(moduleName, module);
    ...
    /*
    * protected method, called if you don't want the widget
    * reference destroyed immediately
    * */

    _getWidget : function(name){
      
      var self = this;
      return  this._getOrCreateBarbarian('widget', name).done(function(w){
        self.__barbarianInstances['widget:' + name].counter+=1;
      });

    },

    /*
    * this will decrement the reference after the done callback has
    * been called
    * */
    ...
    getWidget: function(name) {
      ...
      var self = this;
      var w = this._getWidget(name);

      var done = function done(){
        /*
         * the purpose of this call to returnWidget,
         * which is delayed by setTimeout by 1 millisecond,
         * is to allow the "done" callback to be fired for whatever
         * originally called the getWidget function,
    ...
    _getOrCreateBarbarian: function(cat, name) {
      ...
      //widget isn't here
      if ( (cat == 'widget' && !this.hasWidget(name)) || (cat == 'plugin' && !this.hasPlugin(name)) ) {

        //it's not here, but it's pending, so return the correct promise;
        if (this.pendingWidgets && this.pendingWidgets[name]) {
          return this.pendingWidgets[name];
        }

        //widget hasn't been requested yet
        this.pendingWidgets = this.pendingWidgets || {};
        this.pendingWidgets[name] = d;

        this._loadModule(name).done(function(){
          createBarbarian.apply(self);
        });

      }
      else {
        if ( this._isBarbarianAlive(symbolicName) ){
          d.resolve(this._getBarbarian(symbolicName));
        }
        else {
          createBarbarian.apply(self);
        }
      ...
      function createBarbarian() {
        ...
        if (!(constructor instanceof Function)) {
          console.warn("can't find this widget: " + name); return;
        }
        ...
        //it's been resolved, so delete this reference
        delete this.pendingWidgets[name];
        d.resolve(instance);
      }
    ...
    triggerMethodOnAll: function(funcName, options) {
      ...
      /* BE CAREFUL : this will only return currently instantiated/ loaded widgets*/
