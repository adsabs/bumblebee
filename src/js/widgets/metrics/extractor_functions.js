define([
  ...
  DataExtractor.plot_paperhist = function (options) {
    var data,
      returnArray = [],
      p = options.paperhist_data;

    if (!options.norm) data = [p['refereed publications'], getNonRef(p['refereed publications'], p['all publications'])];
    else data = [p['refereed publications normalized'], getNonRef(p['refereed publications normalized'], p['all publications normalized'])];
  ...
  DataExtractor.plot_readshist = function (options) {
    ...
    if (!options.norm) data = [options.readshist_data['refereed reads'], getNonRef(options.readshist_data['refereed reads'], options.readshist_data['all reads'])];
    else data = [options.readshist_data['refereed reads normalized'], getNonRef(options.readshist_data['refereed reads normalized'], options.readshist_data['all reads normalized'])];
  ...
  DataExtractor.plot_citshist = function (options) {
    ...
    if (!options.norm) data = [c['refereed to refereed'], c['refereed to nonrefereed'], c['nonrefereed to refereed'], c['nonrefereed to nonrefereed']];
    else data = [c['refereed to refereed normalized'], c['refereed to nonrefereed normalized'], c['nonrefereed to refereed normalized'], c['nonrefereed to nonrefereed normalized']];
