...
//       opening the facet and showing its children
      ...
      onClick: function (ev) {
        ...
        if ($(ev.target).parent().hasClass("data-no-interact")){
          ev.preventDefault();
          return
        }
//      select item and its children
        this.$("label:first").toggleClass("s-facet-selected");
/*        make sure child facet state reflects parent state, also
          prevent interaction with child facets for the duration that this item is checked
*/
        if (this.$("label:first").hasClass("s-facet-selected")) {
//      select children, remove checked checkboxes, prevent interaction
          this.$("label").slice(1).addClass("s-facet-selected").addClass("data-no-interact")
            .find("input").prop("checked", false)

        }
        else {
//      unselect children without checked checkboxes, allow interaction
          this.$("label").slice(1).removeClass("s-facet-selected").removeClass("data-no-interact");
        }
