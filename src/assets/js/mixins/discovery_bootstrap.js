...
define([
  ...
  var Mixin = {
    ...
    start: function(Router) {
      ...
      //function to call router every time a relative link is clicked
      $(document).on("click", "a", function(e){
        //prevent app from reloading if it is an internal link
        if (e.currentTarget.host === window.location.host &&
                //check to make sure it wasn't just a "#" url as an event handler
            e.currentTarget.getAttribute('href')
        ){
          //it's internal, trigger the router
          e.preventDefault();
          app.router.navigate(e.currentTarget.getAttribute("href"), { trigger : true });
        }
