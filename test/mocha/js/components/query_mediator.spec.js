    ...
    var test = function () {
      ...
      describe('Query Mediator - Controller (query_mediator.spec.js)', function () {
        ...
        it("should mediate between modules; passing data back and forth", function (done) {
          ...
          var pubsub = beehive.Services.get('PubSub');
          var pubsubSpy = sinon.spy(function () {
            if (debug) {
              console.log('[pubsub:all]', arguments)
            }
          });
          pubsub.on('all', pubsubSpy);
          var api = beehive.Services.get('Api');
          var apiSpy = sinon.spy(function () {
            if (debug) {
              console.log('[api:all]', arguments)
            }
          });
          api.on('all', apiSpy);
        ...
        it("has START_SEARCH signal", function (done) {
          ...
          qm.setApp({
            ...
            getPluginOrWidgetName: function () {
              return false
            },
            hasService: function () {
              return true;
        ...
        it("responds to EXECUTE_REQUEST signal", function (done) {
          var x = createTestQM();
          var qm = x.qm, key1 = x.key1, key2 = x.key2, req1 = x.req1,
            req2 = x.req2;
          qm.activateCache();
          ...
          expect(function () {
            pubsub.publish(key, PubSubEvents.EXECUTE_REQUEST, x.q1)
          }).to.throw.Error;
