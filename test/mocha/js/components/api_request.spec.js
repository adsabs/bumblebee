define([
  'js/components/api_request', 'js/components/multi_params', 'js/components/api_query',
  'backbone'
], function(ApiRequest, MultiParams, ApiQuery, Backbone) {
  ...
  var test = function () {
    describe("ApiRequest (API)", function () {
      ...
      it("has methods for manipulating keys/values", function () {
        ...
        expect(q.previousAttributes()).to.eql({
          'target': 'boo',
          'sender': 'bee'
        });
      ...
      it("can be de/serialized", function () {
        var r = new ApiRequest({
          'target': 'foo',
          'query': new ApiQuery({q: 'foo', b: 'boo'})
        });
        expect(r.url()).to.equal('foo?b=boo&q=foo');

        expect(new ApiRequest({
          target: 'foo',
          sender: 'boo'
        }).url()).to.be.equal('foo#sender=boo');
        expect(new ApiRequest({
          target: 'foo',
          sender: 'boo',
          query: new ApiQuery({q: 'one'})
        }).url()).to.be.equal('foo?q=one#sender=boo');
