    ...
    describe("Discovery mediator (discovery_mediator.spec.js)", function () {
      ...
      var _getM = function () {
        ...
        return {
          app: app,
          dm: mediator,
          reqParseErr: minsub.createRequest({
            target: 'parseerror',
            query: qParseErr,
            sender: 'woo'
          }),
          reqErr: minsub.createRequest({
            target: 'eerror',
            query: qError,
            sender: 'woo'
          }),
          req: minsub.createRequest({
            target: 'search',
            query: q,
            sender: 'woo'
          }),
          reqUnAuthorized: minsub.createRequest({
            target: 'search',
            query: qUnAuthorized,
            sender: 'woo'
          }),
      ...
      it("resets ORCID settings when the ORCID API returns 401, which is passed to the feedback service", function () {
        ...
        var app = {
          ...
          getService: function (service) {
            return minsub.beehive.getService(service)
          },
          getObject: function (_object) {
            return minsub.beehive.getObject(_object)
          }
        };
