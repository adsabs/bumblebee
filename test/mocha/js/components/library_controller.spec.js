define([
...
], function(_, LibraryController, MinSub, JSONResponse) {
  describe('Library Controller', function() {
    ...
    it('should automatically keep the libraries metadata collection in sync throughout different CRUD operations', function() {
      ...
      expect(l.composeRequest.args[1]).to.eql([
        'biblib/libraries',
        'POST',
        {
          data: {
    ...
    it('should allow widgets to create + delete libraries', function() {
      ...
      expect(l.getBeeHive().getService('PubSub').publish.args[2]).to.eql([
        '[Router]-Navigate-With-Trigger',
        'AllLibrariesWidget',
        'libraries',
      ]);

      expect(JSON.stringify(l.getBeeHive().getService('PubSub').publish.args[3])).to.eql(
        JSON.stringify([
          '[Alert]-Message',
          {
            code: 0,
    ...
    it('should allow widgets to add bibcodes to libraries', function() {
      ...
      expect(JSON.stringify(updateLibraryContentsSpy.args[0])).to.eql(
        '["7",{"bibcode":{"bibcodes":["bib1","bib2","bib3"]},"action":"add"}]'
      );
    ...
    it('should have an importLibraries function to import from classic or 2.0', function() {
      ...
      var requestSpy = sinon.spy(function() {
        arguments[0].toJSON().options.done();
      });
