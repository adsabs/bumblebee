define([
  ...
  var createOrcidServer = function(orcidApi, minsub) {
    ...
    if (minsub) {
      server.respondWith('GET', /\/exchangeOAuthCode/, [
        200,
        {
          'Content-Type': 'application/json',
        },
  ...
  describe('Orcid API service (orcid_api.spec.js)', function() {
    describe('OAuth', function() {
        ...
        var minsub = new (MinimalPubsub.extend({
          request: function(apiRequest) {
            if (apiRequest.get('target').indexOf('/exchangeOAuthCode') > -1) {
              ...
              expect(opts.headers['Orcid-Authorization']).to.eql(
                'Bearer 4274a0f1-36a1-4152-9a6b-4246f166bafe'
              );
      ...
      it('has methods to extract access code', function() {
        ...
        expect(oApi.getUrlParameter('code', '?foo=bar&code=H1trXI')).to.eql(
          'H1trXI'
        );
      ...
      it('should handle ORCID sign in', function() {
        ...
        expect(
          JSON.stringify(this.beehive.getService('PubSub').publish.args[0])
        ).to.eql(
          '[{},"[App]-Exit",{"type":"orcid","url":"undefined?scope=/orcid-profile/read-limited%20/orcid-works/create%20/orcid-works/update&response_type=code&access_type=offline&show_login=true&client_id=APP-P5ANJTQRRTMA6GXZ&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fuser%2Forcid"}]'
        );
        expect(
          JSON.stringify(this.beehive.getService('PubSub').publish.args[1])
        ).to.eql('[{},"[PubSub]-Orcid-Announcement","login"]');
      });
    ...
    describe('Orcid Actions', function() {
      ...
      it('should be GenericModule', function(done) {
        expect(new OrcidApi() instanceof GenericModule).to.equal(
          true,
          'is GenericModule'
        );
        expect(new OrcidApi() instanceof OrcidApi).to.equal(
          true,
          'is OrcidApi'
        );
        done();
      ...
      it('exports hardened interface', function() {
        ...
        var check = function(props) {
          _.forEach(props, function(p) {
            expect(_.has(hardened, p)).to.equal(
              true,
              'Hardened Interface has property ' + p
            );
          });
        };
      ...
      it('getUserProfile', function(done) {
        ...
        oApi.getUserProfile().done(function(profile) {
          expect(profile instanceof Profile).to.equal(
            true,
            'returns instance of Profile'
          );
          done();
        });
      });
      ...
      it('getWork', function(done) {
        var oApi = getOrcidApi(this.minsub.beehive);
        createOrcidServer(oApi, this.minsub);
        oApi.getWork(99999).done(function(work) {
          expect(work instanceof Work).to.equal(
            true,
            'returns instance of Work'
          );
          done();
        });
      });
      ...
      it('deleteWork', function(done) {
        var oApi = getOrcidApi(this.minsub.beehive);
        createOrcidServer(oApi, this.minsub);
        oApi.deleteWork(99999).done(function(data, status, xhr) {
      ...
      it('updateWork', function(done) {
        ...
        oApi
          .updateWork(orcidWork)
          .done(function(work) {
            expect(JSON.stringify(work)).to.equal(
              JSON.stringify(orcidWork),
      ...
      it.skip('has methods to query status of a record', function(done) {
        ...
        oApi
          .getRecordInfo({ bibcode: '2018cnsns..56..296s' })
          .done(function(recInfo) {
            expect(oApi._checkIdsInADS.called).to.eql(true);
            expect(oApi._checkIdsInADS.calledTwice).to.eql(true);
            expect(oApi._checkIdsInADS.args[0][0].get('q')).eql([
              'alternate_bibcode:("bibcode-foo" OR "test-bibcode")',
            ]);
            expect(oApi._checkIdsInADS.args[1][0].get('q')).eql([
              'bibcode:("bibcode-foo" OR "test-bibcode")',
            ]);
            ...
            oApi
              .getRecordInfo({ bibcode: 'bibcode-foo' })
              .done(function(recInfo) {
            ...
            oApi
              .getRecordInfo({ doi: '10.1126/science.276.5309.88' }) // doi of bibcode-foo
              .done(function(recInfo) {
                expect(recInfo.isCreatedByADS).to.eql(false);
                expect(recInfo.isCreatedByOthers).to.eql(true);
                expect(recInfo.isKnownToAds).to.eql(true);
            ...
            oApi
              .getRecordInfo({ doi: '10.1103/physrevlett.84.3823' }) // test-bibcode
              .done(function(recInfo) {
                expect(recInfo.isCreatedByADS).to.eql(true);
                expect(recInfo.isCreatedByOthers).to.eql(false);
                expect(recInfo.isKnownToAds).to.eql(true);
            ...
            oApi
              .getRecordInfo({ bibcode: '1997Sci...276...88V' }) // alternate bibcode of bibcode-foo
              .done(function(recInfo) {
                expect(recInfo.isCreatedByADS).to.eql(false);
                expect(recInfo.isCreatedByOthers).to.eql(true);
                expect(recInfo.isKnownToAds).to.eql(true);
            ...
            oApi
              .getRecordInfo({ bibcode: '2015CeMDA.tmp....1D' })
              .done(function(recInfo) {
                expect(recInfo.isCreatedByADS).to.eql(false);
                expect(recInfo.isCreatedByOthers).to.eql(true);
            ...
            // non-ADS record
            oApi
              .getRecordInfo({ bibcode: 'sfasdfsdfsdfsdfsdf' })
              .done(function(recInfo) {
                expect(recInfo.isCreatedByADS).to.eql(false);
                expect(recInfo.isCreatedByOthers).to.eql(false);
                expect(recInfo.isKnownToAds).to.eql(false);
