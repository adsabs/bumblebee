  ...
  describe('Orcid Extension (orcid_extension.spec.js)', function() {
      ...
      minsub = new (MinimalPubsub.extend({
        request: function(apiRequest) {
          var fakeSolrResponse =
            this.requestCounter % 2 == 0 ? Test2() : Test1();
          if (apiRequest.get('query').get('start'))
            fakeSolrResponse.response.start = apiRequest
              .get('query')
              .get('start')[0];
    ...
    it('has methods to manipulate records', function(done) {
      ...
      var check = function(idx, prop, exp) {
        ...
        expect(len === 1).to.eql(
          exp,
          prop + ' on element ' + idx + ' is correct'
        );
      };
    ...
    it('handles a successful getRecordInfo request', function(done) {
      ...
      expect(
        w.widget.view.children.findByIndex(1).$el.find('.s-orcid-loading')
          .length
      ).to.eql(1);
      ...
      expect(
        w.widget.view.children.findByIndex(1).$el.find('.orcid-update').length
      ).to.eql(1);
      done();
    });
    ...
    it('handles a rejected getRecordInfo request', function(done) {
      ...
      _.delay(function() {
        ...
        expect(
          w.widget.view.children.findByIndex(1).$el.find('button.btn-danger')
            .length
        ).to.eql(1);
        done();
      }, 500);
    ...
    it('merges ADS data before sending them to orcid', function(done) {
      ...
      expect(
        widget
          .getPubSub()
          .publish.lastCall.args[1].get('query')
          .get('q')
      ).to.eql(['identifier:foo']);
