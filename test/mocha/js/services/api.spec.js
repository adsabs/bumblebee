  ...
  var test = function () {
    ...
    describe("Api Service (api.spec.js)", function () {
      ...
      it('should accept ApiRequest objects only', function (done) {
        ...
        expect(function () {
          api.request({query: 'q=foo'});
        }).to.throw(Error);
        expect(function () {
          api.request(new ApiRequest({query: 'q=foo'}))
        }).to.throw(Error);
        expect(function () {
            api.url = '';
      ...
      it("should look at the ApiRequest to check whether to send a get (with url data) or post request (with json data)", function (done) {
        ...
        api.request(new ApiRequest({
          target: 'search',
          query: q,
          options: {method: "POST", contentType: 'application/json'}
        }));
        this.server.respond();
      ...
      it("should call appropriate callback upon arrival of data", function (done) {
        ...
        expect(api.request(new ApiRequest({
          target: 'search',
          query: q,
          sender: 'woo'
        }))).to.be.OK;
    ...
    describe("API Service - using live server (api.spec.js)", function () {
      ...
      it("should retrieve data from server using GET and POST (default)", function (done) {
        ...
        defers.push(api.request(new ApiRequest({
          target: 'search',
          query: q,
          sender: 'woo'
        }), {done: f, type: "GET"}));
