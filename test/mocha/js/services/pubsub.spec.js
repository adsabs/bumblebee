define([
  ...
  'backbone',
], function(_, GenericModule, PubSub, PubSubKey, PubSubEvents, Facade, Backbone) {
  describe('PubSub - default implementation of Service (pubsub.spec.js)', function() {
    ...
    it('requires the key to subscribe/unsubscribe', function() {
      ...
      expect(function() {
        p.subscribe({}, 'event', spy);
      }).to.throw(Error);
      expect(function() {
        p.subscribe('event', spy);
      }).to.throw(Error);
      expect(function() {
        p.unsubscribe({}, 'event', spy);
      }).to.throw(Error);
      expect(function() {
        p.unsubscribe('event', spy);
      }).to.throw(Error);
    ...
    it('protects callbacks of other modules', function() {
      ...
      var spy1 = module1.callback,
        spy2 = module2.callback;
    ...
    it('error in callback will not disrupt other callbacks', function() {
      ...
      _.each(_.range(r), function(element, index, list) {
        if (index === r / 2) {
          pubsub.subscribe(k, 'event', function() {
            throw new Error('Testing error handling: ignore me');
          });
        } else {
          pubsub.subscribe(k, 'event', function() {
    ...
    it('undefined events are checked and thrown', function() {
      ...
      expect(function() {
        hardened.publish(undefined, sinon.spy());
      }).to.throw(Error);
      expect(function() {
        pubsub.publish(undefined, sinon.spy());
      }).to.throw(Error);
    });
