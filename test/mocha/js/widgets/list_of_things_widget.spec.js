define([
  ...
  describe('ListOfThings (list_of_things_widget.spec.js)', function() {
    ...
    it('has a pagination view and model that handle displaying and transmitting pagination state and changes to localStorage', function(done) {
      ...
      expect(
        $('#per-page-select>option:selected')
          .text()
          .trim()
      ).to.eql('50');
      ...
      expect(JSON.stringify(widget.model.toJSON())).to.eql(
        '{"mainResults":false,"showAbstract":"closed","showHighlights":"closed","showSidebars":true,"pagination":true,"start":0,"highlightsLoaded":false,"perPage":50,"numFound":100,"currentQuery":{"q":["foo:bar"]},"pageData":{"perPage":50,"totalPages":2,"currentPage":1,"previousPossible":false,"nextPossible":true},"focusedIndex":-1,"page":0,"showRange":[0,49],"query":false,"loading":false}'
      );
    ...
    it("the item view allows the user to view the lsit in a search results page if 'operator' option is true and 'queryOperator' option is set", function() {
      ...
      const el = $('#test .s-operator');
      const expText = normalizeSpace($('a', el).attr('href') + ' ' + el.text());

      expect(expText).to.eql(
        '#search/q=citations(bibcode%3Afoo) view this list in a search results page'
      );
    });
