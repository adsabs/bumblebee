...
define([
...
], function(_, Backbone, Marionette, $, AbstractWidget, BaseWidget, MinimalPubSub) {
  describe('Abstract Renderer (abstract_widget.spec.js)', function() {
    ...
    beforeEach(function() {
          ...
          docs: [
            {
              ...
              abstract:
                'In the past twenty years there has been a great amount of growth in radiometric observing methods.',
      ...
      var fakeAppStorage = {
        ...
        getCurrentQuery: function() {
          return new MinimalPubSub.prototype.T.QUERY();
        },
      };
    ...
    it("should show an appropriate default page in the case that the bibcode isn't found", function() {
      ...
      expect(
        $('article')
          ...
          .trim()
          .replace(/\s{2}/gi, '')
      ).to.eql('Abstract Not FoundNo valid abstract selected for retrieval or abstract not yet indexed in ADS.');
    });
    ...
    it("should render a view with the properly rendered information and 'view more' user interactions", function() {
      ...
      expect($w.find('.s-abstract-text').text()).to.match(
        /In the past twenty years there has been a great amount of growth in radiometric observing methods./
      );
    });
    ...
    it.skip('should pre-populate with values from the docstashcontroller', function() {
      ...
      expect(JSON.stringify(_.values(aw._docs)[0])).to.eql(
        JSON.stringify({
          bibcode: 'baz',
          keyword: ['HARMONY OF THE UNIVERSE', 'THEORY OF MUSIC', "PLATO'S BODIES"],
          author: ['Lieske, J. H.', 'Standish, E. M.'],
          ...
          hasAffiliation: 2,
          authorAff: [
            ['Lieske, J. H.', 'Heidelberg, Universität, Heidelberg, Germany', '%22Lieske%2C+J.+H.%22'],
            [
              'Standish, E. M.',
              'California Institute of Technology, Jet Propulsion Laboratory, Pasadena, CA',
              '%22Standish%2C+E.+M.%22',
