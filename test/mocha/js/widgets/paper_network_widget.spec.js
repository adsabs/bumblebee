...
//    $("#test").append(paperNetwork.view.el);
    ...
    it("should display a view with a list of most significant common references when a link between groups is selected", function () {
      ...
      var group1 = d3.selectAll(".node-path").filter(function (d) {
        return d.data.id == 0
      })[0][0].__data__;
      var group2 = d3.selectAll(".node-path").filter(function (d) {
        return d.data.id == 1
      })[0][0].__data__;
    ...
    it("has a filter capability that allows you to add or remove groups or individual nodes, and submit as a filter", function () {
      ...
      expect(paperNetwork.getPubSub().publish.args[0][0]).to.eql("[PubSub]-Custom-Event");
      const expectedBibs = [
        "2004AJ....128.1078R",
        "1999ApJ...517L..23G",
        "2003ASPC..295...51M",
      ...
      const actualBibs = paperNetwork.getPubSub().publish.args[0][2].ids || [];
      expect(actualBibs.sort()).to.eql(expectedBibs.sort());
    });
  });
});
