define([
  ...
  'js/widgets/list_of_things/expanding',
  'js/components/api_query',
  './test_json/test1',
  './test_json/test2',
  'js/widgets/base/paginated_base_widget',
], function(_, Marionette, Backbone, MinimalPubsub, ListOfThingsWidget, ApiQuery, Test1, Test2, PaginatedBaseWidget) {
    ...
    it('should listen to INVITING_REQUEST event', function(done) {
      ...
      expect(
        $w
          ...
          .eq(0)
          .text()
      ).to.equal('2013arXiv1305.3460H');

      minsub.publish(
        minsub.INVITING_REQUEST,
        new ApiQuery({
          q: 'star',
          x: 'bar', // different query = no cache
        })
      );
